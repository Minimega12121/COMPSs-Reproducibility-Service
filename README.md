...
- Ensure that all dependencies for the experiment you wish to reproduce are satisfied on your local machine.

## How to Use

- Take the link to the workflow or the path to the folder of the RO-Crate (or a zip file) and pass it as the first argument to the service:
  ```bash
...
2. **New Dataset Feature**: If you want to reproduce the same experiment with a new dataset, simply provide the path to the new dataset.
   > **Note**: The new dataset should follow a similar directory structure as the old one for the paths to be correctly mapped.

3. **Flag Addition**: You can review the `runcompss` command line generated by the service and request additional flags according to your needs.
...
- No automatic detection or loading of third-party dependencies on a SLURM cluster.
- No support for workflows with `data_persistence=false` with all datasets as remote files.
- `data_persistence=false` examples only work on a SLURM cluster where paths related to the experiment are accessible.
